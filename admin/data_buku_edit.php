...
$data_buku = mysqli_fetch_assoc($pilih);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
    if ($ukuran_sampul < 1044070) {
      ...
      $query = mysqli_query($koneksi, "UPDATE buku SET  isbn='$isbn', judul='$judul', penulis_id='$penulis_id', penerbit_id='$penerbit_id', kategori_id='$kategori_id',tahun_terbit='$tahun_terbit', sinopsis='$sinopsis', jumlah='$jumlah', foto_sampul='$nama_sampul' WHERE id_buku='$id'");
          ...
          <div class="table-responsive">
            <form action="" method="post" enctype="multipart/form-data">
              <div class="mb-3">
                ...
                <input type="text" id="isbn" name="isbn" class="form-control" required value="<?php echo $data_buku['isbn'] ?>">
              </div>
              <div class="mb-3">
                <label for="judul" class="judul">Judul</label>
                <input type="text" id="judul" name="judul" class="form-control" required value="<?php echo $data_buku['judul'] ?>">
              </div>
              <div class="mb-3">
                ...
                <?php if (mysqli_num_rows($penulis) > 0) : ?>
                  <select name="penulis_id" class="form-select">
                    <?php while ($data_penulis = mysqli_fetch_array($penulis)) : ?>
                      <option value="<?php echo $data_penulis['id_penulis'] ?>" <?php if ($data_penulis['id_penulis'] == $data_buku['penulis_id']) { ?> selected="selected" <?php } ?>>
                ...
                <?php if (mysqli_num_rows($penerbit) > 0) : ?>
                  <select name="penerbit_id" class="form-select">
                    <?php while ($data_penerbit = mysqli_fetch_array($penerbit)) : ?>
                      <option value="<?php echo $data_penerbit['id_penerbit'] ?>" <?php if ($data_penerbit['id_penerbit'] == $data_buku['penerbit_id']) { ?> selected="selected" <?php } ?>>
                ...
                <?php if (mysqli_num_rows($kategori) > 0) : ?>
                  <select name="kategori_id" class="form-select">
                    <?php while ($data_kategori = mysqli_fetch_array($kategori)) : ?>
                      <option value="<?php echo $data_kategori['id_kategori'] ?>" <?php if ($data_kategori['id_kategori'] == $data_buku['kategori_id']) { ?> selected="selected" <?php } ?>>
              ...
              <div class=" mb-3">
                ...
                <input name="tahun_terbit" class="form-control" type="number" min="1900" max="2100" placeholder="2020" required value="<?php echo $data_buku['tahun_terbit'] ?>">
              </div>
              <div class="mb-3">
                <label for="sinopsis" class="form-label">sinopsis</label>
                <input type="text" id="sinopsis" name="sinopsis" class="form-control" required value="<?php echo $data_buku['sinopsis'] ?>">
              </div>
              <div class="mb-3">
                ...
                <input type="text" id="jumlah" name="jumlah" class="form-control" required value="<?php echo $data_buku['jumlah'] ?>">
              </div>
              <div class="mb-3">
                <label for="foto_sampul" class="form-label">Foto Sampul</label>
                <input type="file" id="foto_sampul" name="foto_sampul" class="form-control" required accept="image/*" value="<?php echo $data_buku['foto_sampul'] ?>">
              </div>
